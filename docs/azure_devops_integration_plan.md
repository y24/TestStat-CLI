...
- プロジェクト新規作成画面で、Testing ID を入力し「AzureDevOpsから情報取得」ボタンを押すと、以下を自動入力する。
...
[ブラウザ / teststat-frontend]
   新規プロジェクト作成画面
   Testing ID を入力 →「AzureDevOpsから情報取得」ボタン
...
新規作成モード（`mode === 'new'`）で、Testing ID 入力欄の近くに「**AzureDevOpsから情報取得**」ボタンを追加する。
...
```
Testing ID  [  1001     ] [AzureDevOpsから情報取得]
...
- [ ] `ProjectEditor.tsx` に「AzureDevOpsから情報取得」ボタン＋反映・エラー・ローディング処理
- [ ] 既存ローカル実績自動補完との整合確認
