...
testings（testing_id ごとに 1 レコード, project_name を持つ）   ← CLI が UPSERT
  └─ file_progress        … ファイル(テスト種別/environment)単位のサマリ
...
| `GET` | `/progress/{testing_id}/files` | ファイル(テスト種別)別サマリ一覧 |
| `GET` | `/progress/{testing_id}/daily` | 日別実績明細（date×テスト種別 の completed/executed/planned/結果種別） |
| `GET` | `/health` | ヘルスチェック |
...
| テスト種別 | プロジェクト内の個別テスト種別 | `label`（実績の file_progress.label に対応） | **フロント新設（`plans` の単位）** |
| 計画バージョン | テストごとの計画（期間・項目数・日別配分） | version | **フロント新設（`plans`/`plan_daily`）** |
| 実績 | CLI が送る消化・完了・結果 | `testing_id`＋`label`＋`date` | CLI（既存テーブル） |

- **プロジェクト ↔ `testing_id` は 1:1。** 実際の CLI 運用（`list_sample.yaml`）では 1 つの `testing_id` の下に複数ファイル（`label` = TEST001/002/003）がぶら下がる構造と確認済み。
  → **「プロジェクト=チケット=testing_id、テスト種別=label」で確定**（ユーザー確認済み）。
  （将来「1プロジェクトが複数 testing_id にまたがる」構成が必要になれば §10 の拡張で対応。今回は 1:1。）
- **計画はテスト種別単位**で作成し、**プロジェクト表示時は各テストの有効計画を合算**する（壁打ちメモ §9 の構造を採用）。

### 2.2 実績は1件・計画は多バージョン

```
projects（testing_id 1件）
  └─ plans（テスト種別ごとに複数バージョン。is_active が現行）
       └─ plan_daily（日別の計画消化数）
...
| 全テスト種別 / 単一テスト種別切替 | ✅ 採用 | テスト種別で計画も実績も絞り込み／合算。 |
...
| **過去の計画を薄く重ねて表示** | ✅ **採用** | 計画をバージョン管理するため履歴が残る。非有効版を薄線で重ねられる。 |
...
### 4.2 `plans`（テスト種別ごとの計画バージョン）

| カラム | 型 | 説明 |
|--------|-----|------|
| `id` | SERIAL PK | |
| `testing_id` | INTEGER NOT NULL | `projects.testing_id` を参照 |
| `label` | VARCHAR(255) NULL | テスト種別。NULL はプロジェクト全体を1テストとして計画する場合 |
...
| メソッド | パス | 用途 |
|---------|------|------|
| `GET` | `/projects/{testing_id}/plans` | 計画一覧（テスト種別別・全バージョン） |
| `POST` | `/projects/{testing_id}/plans` | 計画線の作成（下記ボディ） |
...
| メソッド | パス | クエリ |
|---------|------|--------|
| `GET` | `/projects/{testing_id}/pb-chart` | `label`（任意。未指定=全テスト種別合算）, `include_past_plans`（任意 bool） |
...
サンプルプロジェクト > テスト計画
┌──────────────┬──────┬───────────────┬──────────┬────────┐
│ テスト種別    │ 項目数 │ 期間          │ 有効な計画 │ 操作    │
├──────────────┼──────┼───────────────┼──────────┼────────┤
│ TEST001      │  920 │ 05/01 - 06/10 │ v2       │[編集][履歴]│
│ TEST002      │ 1200 │ 05/08 - 06/12 │ v1       │[編集][履歴]│
└──────────────┴──────┴───────────────┴──────────┴────────┘
```
- テスト種別は、実績がある場合 `/files` の label から候補提示。無ければ手入力（実績到着時にテスト種別の文字列一致で紐付くため、表記を合わせる旨を注意表示）。
- 計画線の作成フォーム: 変更理由・テスト項目数・開始日・終了日・入力方法。
...
| F4. 仕上げ | 全テスト種別/単一テスト種別切替、過去計画トグル、欠損・負値・実績未受信の表示、ローディング/エラー | 異常系も破綻しない |
...
2. ~~**テスト種別単位 vs プロジェクト単位の計画**~~ → **確定**: テスト種別 = ファイルの `label` で正しい。**テスト種別単位で計画を管理する設計を採用**（§2.1・§4.2）。
...
6. **テスト種別の表記揺れ対策**: 計画の label と実績の label は文字列一致で紐付く。実績がある場合は候補から選ばせ、手入力時は注意喚起する。
```
